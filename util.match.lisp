...
(cl:in-package :util.match-internal)

(def-suite util.match)

(in-suite util.match)

;;; "util.match" goes here. Hacks and glory await!
...
;;
(defvar match|:|version "Version 1.18, July 17, 1995")
;;
...
(define match|:|error
  (lambda (val . args)
    (error "~a: no matching clause for ~a"
           (format nil "~{~A~^ ~}" args)
            val)))

(define match|:|every #'srfi-1:every) ; alias srfi-1's every, in case the user of
                           ; util.match isn't using srfi-1.

(define match|:|syntax-err
        (lambda (obj msg) (error msg obj)))

(defvar match|:|error-control 'error)
...
(defvar match|:|disjoint-predicates
  (cons 'null
        '(pair?
          symid?
          boolean?
...
(define (dot-dot-k? s)
  (and (symid? s)
       (if (member s '(*** ___))
         0
         (and-let* ((s (and (symbol? s) (string s)))
                    ((or (and (char= #\* (char s 0))
                              (char= #\* (char s 1)))
                         (and (char= #\_ (char s 0))
                              (char= #\_ (char s 1))))))
           (values (parse-integer s :start 2))))))

(test dot-dot-k
  (is (= 0 (dot-dot-k? '***)))
  (is (= 1 (dot-dot-k? '**1)))
  (is (= 100 (dot-dot-k? '**100))))

(define (error-maker match-expr)
  (cond
    ((eq? match|:|error-control 'unspecified)
     (cons '() (lambda (x) (declare (ignore x))`(cond (|#F| |#F|)))) )
 ...
 (letrec ((simple?
           (lambda (x)
             (or (string? x) (boolean? x)
                 (char? x) (number? x) (null? x) (keyword? x))))
          (ordinary
           (lambda (p)
                  ...
                  (case (car p)
                    ((quasiquote)
                     (if (and (pair? (cdr p)) (null? (cddr p)))
                         (quasi (cadr p))
                         (funcall cons-ordinary (car p) (cdr p)) ))
                    ((quote)
                     (if (and (pair? (cdr p)) (null? (cddr p)))
                         p
                         (funcall cons-ordinary (car p) (cdr p)) ))
                    ((?)
                     (if (and (pair? (cdr p)) (listp (cddr p)))
                    ...
                    ((not)
                     (if (and (listp (cdr p)) (pair? (cdr p)))
                         ...
                         (funcall cons-ordinary (car p) (cdr p)) ))
                    (($ struct)
                     (if (and (pair? (cdr p)) (symid? (cadr p)) (listp (cddr p)))
                         `($ ,(cadr p) ,@(map #'ordinary (cddr p)))
                         (funcall cons-ordinary (car p) (cdr p)) ))
                    ((@ object)
                     (if (and (pair? (cdr p)) (symid? (cadr p)) (listp (cddr p))
                              (every (lambda (p) (and (listp p) (= (length p) 2)))
                                     (cddr p) ))
                         `(object ,(cadr p)
                                  ,@(map (lambda (p)
                 ...
                 ((pair? p)
                  (cond
                    ((eq? (car p) 'unquote)
                     (if (and (pair? (cdr p)) (null? (cddr p)))
                         (ordinary (cadr p))
                         (funcall cons-quasi (car p) (cdr p)) ))
                    ((and (pair? (car p))
                    ...
                    ((and (pair? (cdr p))
                          ...
                          (null? (cddr p)) )
                     `(,(quasi (car p)) ,(cadr p)) )
                    (:else (funcall cons-quasi (car p) (cdr p))) ))
                 ((vectorp p)
                  (let* ((pl (coerce p 'list))
                         (rpl (reverse pl)) )
                    (coerce
             ...
             (cond
               ((or (not (symid? (cadr p)))
                ...
                (let ((g (gensym)))
                  (push `(,g ,(cadr p)) pred-bodies)
                  (bound `(= ,g ,(caddr p)) a k) ))
               (:else (bound (caddr p) a (lambda (p2 a) (funcall k `(= ,(cadr p) ,p2) a)))) ))
            ((and (pair? p) (eq? 'and (car p)))
             (bound* (cdr p) a (lambda (p a) (funcall k `(and ,@p) a))) )
            ((and (pair? p) (eq? 'or (car p)))
             (bound (cadr p) a
                    (lambda (first-p first-a)
                      (srfi-5:let or* ((plist (cddr p))
                                       (k (lambda (plist) (funcall k `(or ,first-p ,@plist)
                                                             first-a ))))
                           (if (null? plist)
                               (funcall k plist)
                               (bound
                    ...
                    (lambda (q b)
                      (let ((bvars (find-prefix b a)))
                        (funcall k `(,q ,(cadr p)
                                ...
                                ,(gensym)
                                ,(gensym)
                                ,(map (lambda (_) (declare (ignore _)) (gensym)) bvars) )
                           b )))))
            ((and (pair? p) (memq (car p) '($ struct)))
             (bound* (cddr p)
                     a
                     (lambda (p1 a)
                       (funcall k `($ ,(cadr p) ,@p1) a) )))
            ...
            ((pair? p)
             (bound (car p)
                    ...
                    (lambda (car-p a)
                      (bound (cdr p)
                             a
                             (lambda (cdr-p a) (funcall k (cons car-p cdr-p) a)) ))))
            ((vector? p)
             (boundv (vector->list p)
                     a
                     (lambda (pl a) (funcall k (list->vector pl) a)) ))
            (:else (funcall k p a)) )))
       ;;
       (boundv
        (lambda (plist a k)
          (let ((g115 (lambda () (funcall k plist a))))
            (if (pair? plist)
       ...
       (permutation
        (lambda (p1 p2)
          (and (= (length p1) (length p2))
               (every (lambda (x1) (member x1 p2)) p1) ))))
...
;   (erract x)
;   (let* ((v '())
;          (val (lambda (x) (cdr (assoc x v :test #'eq))))
;          (fail (lambda (sf)
...
;                                 ,ap))))
;                         `(,code ,@(map val bv)))))))
;     (srfi-5:let next ((p (caar plist))
;                       (e x)
;                       (sf sf)
;                       (kf fail)
;                       (ks success))
;       (cond
;        ((eq? '_ p) (funcall ks sf))
